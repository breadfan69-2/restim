...
from device.focstim.notifications_pb2 import (NotificationBoot, NotificationDeviceVolume, NotificationCurrents, \
    ...
    NotificationBattery, NotificationLSM6DSOX, NotificationButtonPress, NotificationDebugString, \
    NotificationDebugAS5311, NotificationPressure, NotificationDebugTeleplot)
...
class FOCStimProtoDevice(QObject, OutputDevice):
    ...
    def handle_notification_skin_resistance(self, notif: NotificationSkinResistance):
        a = notif.resistance_a + 1j * notif.reluctance_a
        b = notif.resistance_b + 1j * notif.reluctance_b
        c = notif.resistance_c + 1j * notif.reluctance_c
        d = notif.resistance_d + 1j * notif.reluctance_d
    ...
    def handle_notification_battery(self, notif: NotificationBattery):
        ...
        self.new_battery_data.emit(int(np.round(notif.battery_soc * 100)))
